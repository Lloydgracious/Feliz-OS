...
export default function BraceletPreview({ colors = [], knotLabel }) {
  const displayColors = colors.length > 0 ? colors : [{ name: 'Sky Blue', hex: '#7cc7ff' }, { name: 'White Jade', hex: '#ffffff' }]

  // Generate gradient string for conic-gradient
  const gradientParts = []
  const slice = 100 / displayColors.length
  displayColors.forEach((c, i) => {
    const start = i * slice
    const end = (i + 1) * slice
    gradientParts.push(`${c.hex} ${start}% ${end}%`)
  })
  const conicBg = `conic-gradient(from 120deg, ${gradientParts.join(', ')})`
  ...
  return (
    <div className="lux-glass rounded-[32px] p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="mt-1 text-sm text-slate-700">
            {displayColors.map(c => c.name).join(' + ')}
          </div>
        </div>
      ...
      <div className="mt-7 grid place-items-center">
        <motion.div
          key={displayColors.map(c => c.hex).join('-')}
        ...
        >
          ...
          <div
            ...
            style={{
              background: `linear-gradient(145deg, ${displayColors[0]?.hex}cc, ${displayColors[displayColors.length - 1]?.hex}55)`,
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 text-sm text-slate-700">
        {displayColors.slice(0, 4).map((c, i) => (
          <div key={i} className="rounded-2xl border border-white/20 bg-white/20 px-4 py-3">
            <div className="text-xs text-slate-600">{i === 0 ? 'Primary' : i === 1 ? 'Secondary' : `Color ${i + 1}`}</div>
